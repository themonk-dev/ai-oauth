...
The value that survived is a live refresh token, and where it survived to is the message of the thrown `OAuthError` — built by `safeSnippet()` from the response body — which goes on to the consumer's logs, crash reporter and terminal scrollback. Both branches of `readTokenError` were affected, the JSON one and the plain-text one.
